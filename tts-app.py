...
def trim_text(text, max_length=30):
    ...
    """
    if len(text) > max_length:  #or use # return text[:max_length] + "..." if len(text) > max_length else text
...
def generate_speech_with_timestamps(text, speaker, language):
    ...
    # Note: The above can also be written as a one-liner for brevity:
    # selected_language = 'en' if language == "US English" else 'es'
